...
## Decision
Use symlinks in `~/bin/` pointing to scripts in the repository folder. Scripts remain in the repository (version-controlled), and symlinks provide system-wide access. The `gitscripts_setup_symlinks` script automates symlink creation and updates.
...
**Risks:**
- Users moving repository without updating symlinks (mitigated by `gitscripts_setup_symlinks` detecting and fixing this)
- PATH configuration issues (mitigated by script automatically adding `~/bin/` to PATH)

## Links
- Implementation: `scripts/gitscripts_setup_symlinks`
- Used by: `environment_install`, `gitscripts_install`
